...
########################## Task 1 ###############################
    ...
    def fullDeviceList(self, filename):
        ...
        vapi.dict2csv(devices, filename, fields)
...
########################## Task 2 ###############################
    ...
    def custom_DeviceList(self, filename, inputHeadersfile):
        ...
        devices = vapi.getDevices(self)
        vapi.dict2csv(devices, filename, newDictHeaders)
...
########################## Task 3 ###############################
    def getFabricHealth(self, filename):
        ...
        vapi.dict2csv(data, filename, fields)
    ...
    #         new_device = {}
    #         try:
    #             if len(device['system-ip']) > 2:
    #                 new_device['site-id'] = device['site-id']
    #                 new_device['system-ip'] = device['system-ip']
    #                 new_device['host-name'] = device['host-name']
    #                 new_device['reachability'] = device['reachability']
    #         except:
    #             continue
    ...
    def getControlConn(self, devices):
        # devices = vapi.generateDeviceList(self)
        for device in devices:
...
system-ip: {device["system-ip"]}
device uptime: {vapi.unixTimeStamp(device["uptime-date"])}
last updated: {vapi.unixTimeStamp(device["lastupdated"])}
...
downtime date: {vapi.unixTimeStamp(data['downtime-date'])}
lastupdated: {vapi.unixTimeStamp(data['lastupdated'])}
        """
        return templateText
...
last connection time: {vapi.unixTimeStamp(data['last-conn-time-date'])}
lastupdated: {vapi.unixTimeStamp(data['lastupdated'])}
        """
        return templateText
...
----------------------------------------
    ...
    def bfdTemplate2(self, data):
        templateText = f"""
====> {data['vdevice-dataKey']} - {data['state']} - Last updated: {vapi.unixTimeStamp(data['lastupdated'])}"""
        return templateText
...
###################### Tools ###########################
    def dict2csv(dict, filename, fields):
        new_dict = []
        for item in dict:
            temp_dict = {}
            for field in fields:
